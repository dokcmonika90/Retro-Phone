...
struct Emulator {
    ...
    uint8_t read(uint16_t a) {
        ...
        if(a>=0x8000 && !prg.empty()) {
            size_t o=a-0x8000; if(prg.size()==0x4000) o%=0x4000; else o%=prg.size(); return prg[o];
        }
        return 0;
    }
    void write(uint16_t a,uint8_t v) {
        ...
        if(a==0x4016){ controllerStrobe=(v&1)!=0; if(controllerStrobe) controller=controller; return; }
    ...
    void updateAudio(uint16_t a,uint8_t v){
        ...
        if(a==0x4002 || a==0x4003){ uint16_t lo=read(0x4002), hi=read(0x4003)&7; uint16_t period=lo|(hi<<8); audioFreq=period?1789773.0/(16.0*(period+1)):0; }
    }

    void setZN(uint8_t v){ if(v==0)p|=2;else p&=~2; if(v&0x80)p|=0x80;else p&=~0x80; }
    ...
    void stepInterp(){
        ...
        case 0x69:{uint16_t s=cpu.a+imm()+((p>>0)&1);p=(p&~1)|((s>255)?1:0);cpu.a=s;setZN(cpu.a);break;}
        case 0xE9:{uint16_t s=cpu.a-imm()-((p>>0)&1?0:1);p=(p&~1)|((s<256)?1:0);cpu.a=s;setZN(cpu.a);break;}
        case 0x29:cpu.a&=imm();setZN(cpu.a);break; case 0x09:cpu.a|=imm();setZN(cpu.a);break; case 0x49:cpu.a^=imm();setZN(cpu.a);break;
        case 0xC9:{uint8_t v=imm();uint8_t r=cpu.a-v;if(cpu.a>=v)p|=1;else p&=~1;setZN(r);break;}
        case 0xE0:{uint8_t v=imm();uint8_t r=cpu.x-v;if(cpu.x>=v)p|=1;else p&=~1;setZN(r);break;}
        case 0xC0:{uint8_t v=imm();uint8_t r=cpu.y-v;if(cpu.y>=v)p|=1;else p&=~1;setZN(r);break;}
        case 0x4C:cpu.pc=abs();break; case 0x20:{uint16_t t=abs();uint16_t r=cpu.pc-1;push(r>>8);push(r);cpu.pc=t;break;}
        case 0x60:cpu.pc=pop()|(pop()<<8);cpu.pc++;break; case 0x40:{p=pop();cpu.pc=pop()|(pop()<<8);break;}
        ...
        case 0xD0:{int8_t o=(int8_t)imm();if(!(p&2))cpu.pc+=o;break;} case 0xF0:{int8_t o=(int8_t)imm();if(p&2)cpu.pc+=o;break;}
        case 0x10:{int8_t o=(int8_t)imm();if(!(p&0x80))cpu.pc+=o;break;} case 0x30:{int8_t o=(int8_t)imm();if(p&0x80)cpu.pc+=o;break;}
        case 0x90:{int8_t o=(int8_t)imm();if(!(p&1))cpu.pc+=o;break;} case 0xB0:{int8_t o=(int8_t)imm();if(p&1)cpu.pc+=o;break;}
        case 0x18:p&=~1;break; case 0x38:p|=1;break; case 0x58:p&=~4;break; case 0x78:p|=4;break;
        case 0x00: p|=0x14; break;
        default: break;
        }
    }
    ...
    static uint32_t ldrx(int rt,int rn,int imm){return 0xF9400000u|((uint32_t)imm<<10)|(rn<<5)|rt;}
    static uint32_t addi(int rd,int rn,int imm){return 0x11000000u|((uint32_t)imm<<10)|(rn<<5)|rd;}
    static uint32_t subi(int rd,int rn,int imm){return 0x51000000u|((uint32_t)imm<<10)|(rn<<5)|rd;}
    static uint32_t movw(int rd,int rn){return 0x2A000000u|(rn<<16)|(rn<<5)|rd;}
    static uint32_t addx(int rd,int rn,int rm){return 0x8B000000u|(rm<<16)|(rn<<5)|rd;}
    bool compileBlock(uint16_t pc){
        if(jitIndex[pc]>=0) return true;
        std::vector<uint32_t> code; uint16_t cur=pc; int count=0;
        auto emit=[&](uint32_t v){code.push_back(v);};
        while(count++<24){
            uint8_t op=read(cur);
            switch(op){
                case 0xEA: // NOP
                    emit(ldrh(3,0,offsetof(CPU,pc)/2)); emit(addi(3,3,1)); emit(strh(3,0,offsetof(CPU,pc)/2)); cur++; break;
                case 0xE8: // INX
                    emit(ldrb(1,0,offsetof(CPU,x))); emit(addi(1,1,1)); emit(strb(1,0,offsetof(CPU,x))); emit(ldrh(3,0,offsetof(CPU,pc)/2)); emit(addi(3,3,1)); emit(strh(3,0,offsetof(CPU,pc)/2)); cur++; break;
                case 0xCA: // DEX
                    emit(ldrb(1,0,offsetof(CPU,x))); emit(subi(1,1,1)); emit(strb(1,0,offsetof(CPU,x))); emit(ldrh(3,0,offsetof(CPU,pc)/2)); emit(addi(3,3,1)); emit(strh(3,0,offsetof(CPU,pc)/2)); cur++; break;
                case 0xAA: // TAX
                    emit(ldrb(1,0,offsetof(CPU,a))); emit(strb(1,0,offsetof(CPU,x))); emit(ldrh(3,0,offsetof(CPU,pc)/2)); emit(addi(3,3,1)); emit(strh(3,0,offsetof(CPU,pc)/2)); cur++; break;
                case 0x8A: // TXA
                    emit(ldrb(1,0,offsetof(CPU,x))); emit(strb(1,0,offsetof(CPU,a))); emit(ldrh(3,0,offsetof(CPU,pc)/2)); emit(addi(3,3,1)); emit(strh(3,0,offsetof(CPU,pc)/2)); cur++; break;
                default: goto done;
            }
        }
    ...
    void reset(){cpu=CPU{};cpu.mem=cpu.ram; std::fill(std::begin(cpu.ram),std::end(cpu.ram),0);ppuStatus=0xA0;ppuCtrl=ppuMask=0;addrLatch=false;ppuAddr=0;std::fill(std::begin(vram),std::end(vram),0);std::fill(std::begin(palette),std::end(palette),0);std::fill(frame.begin(),frame.end(),0xff000000);jitIndex.fill(-1);}

    uint32_t color(uint8_t i){static const uint32_t pal[64]={0xff666666,0xff002a88,0xff1412a7,0xff3b00a4,0xff5f006b,0xff7e003c,0xff7b0800,0xff5c1d00,0xff344000,0xff0b4f00,0xff005500,0xff00504a,0xff003c63,0xff000000,0xff000000,0xff000000,0xffadadad,0xff155fd9,0xff4240ff,0xff7527fe,0xffa01acc,0xffb71e5b,0xffb52f16,0xff994800,0xff6b6d00,0xff388700,0xff0d9300,0xff008b7a,0xff00749e,0xff000000,0xff000000,0xff000000,0xfffffeff,0xff64b0ff,0xff9290ff,0xffc76fff,0xfff36aff,0xfffe6e92,0xffff9b5b,0xfffdbd43,0xffcfe04a,0xff9af57a,0xff5cf0b0,0xff5de1d4,0xff60cfff,0xff808080,0xff000000,0xff000000,0xfffffeff,0xffc0dfff,0xffd0d0ff,0xffe8bfff,0xffffb8,0xffffb8ff,0xffffcfa0,0xffffe0a0,0xffe8f090,0xffc8ff80,0xffa0ffc0,0xffa0fff0,0xffa0e8ff,0xffc0c0c0,0xff000000,0xff000000}; return pal[i&63];}

    void render(){
        uint8_t base=palette[0]; for(int y=0;y<H;y++) for(int x=0;x<W;x++){ int tx=(x+scrollX)>>3,ty=(y+scrollY)>>3; uint8_t tile=vram[(ty*32+tx)&0x3ff]; int px=(x+scrollX)&7,py=(y+scrollY)&7; int plane=(ppuCtrl&0x10)?0:0x1000; int off=tile*16+py; uint8_t lo=chr.empty()?0:chr[off%chr.size()]; uint8_t hi=chr.empty()?0:chr[(off+8)%chr.size()]; uint8_t bit=((lo>>(7-px))&1)|(((hi>>(7-px))&1)<<1); uint8_t pal=palette[(bit==0?0:((tile>>2)&3)*4+bit)&31]; frame[y*W+x]=color(pal); }
        ppuStatus|=0x80;
    }

    void runFrame(){
        if(!loaded){ renderDemo(); return; }
        runCPU(1789773/60/3); render(); ppuStatus|=0x80;
    }
...
extern "C" JNIEXPORT jstring JNICALL Java_com_dokcmonika90_retrophone_MainActivity_nativeVersion(JNIEnv* e,jobject){return e->NewStringUTF("NES renderer + audio + ARM64 dynamic recompiler 0.2");}
