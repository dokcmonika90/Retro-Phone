...
struct Emu{
 CPU c{};std::vector<uint8_t>prg,chr;bool loaded=false;uint8_t ppuctrl=0,ppustatus=0,oam[256]{},vram[4096]{},pal[32]{},buttons=0;bool strobe=false;std::array<int32_t,65536>ji{};std::vector<Block>jb;std::array<uint32_t,W*H>fb{};double phase=0,freq=0;uint8_t vol=0;std::mutex m;
 Emu(){ji.fill(-1);}~Emu(){for(auto&b:jb)munmap(b.mem,b.size);}
 uint8_t rd(uint16_t a){if(a<0x2000)return c.ram[a&2047];if(a<0x4000)return ppuRd(0x2000|(a&7));if(a==0x4016)return pad();if(a>=0x8000&&!prg.empty()){size_t o=a-0x8000;return prg[o%prg.size()];}return 0;}
 void wr(uint16_t a,uint8_t v){if(a<0x2000){c.ram[a&2047]=v;return;}if(a<0x4000){ppuWr(0x2000|(a&7),v);return;}if(a==0x4014){for(int i=0;i<256;i++)oam[i]=rd((uint16_t(v)<<8)+i);return;}if(a==0x4016){strobe=v&1;return;}if(a==0x4000)vol=v&15;if(a==0x4002)freq=1789773.0/(16.0*(uint16_t(v)+1));}
 ...
 bool jit(uint16_t pc){if(ji[pc]>=0)return true;std::vector<uint32_t>v;uint16_t q=pc;for(int k=0;k<16;k++){uint8_t o=rd(q);if(o==0xea){v.push_back(ldh(3,0,offsetof(CPU,pc)/2));v.push_back(0x11000463);v.push_back(sth(3,0,offsetof(CPU,pc)/2));q++;}else if(o==0xe8){v.push_back(ldb(1,0,offsetof(CPU,x)));v.push_back(0x11000421);v.push_back(stb(1,0,offsetof(CPU,x)));v.push_back(ldh(3,0,offsetof(CPU,pc)/2));v.push_back(0x11000463);v.push_back(sth(3,0,offsetof(CPU,pc)/2));q++;}else break;}v.push_back(0xd65f03c0);size_t bytes=v.size()*4,page=(bytes+4095)&~size_t(4095);void*p=mmap(nullptr,page,PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANONYMOUS,-1,0);if(p==MAP_FAILED)return false;memcpy(p,v.data(),bytes);__builtin___clear_cache((char*)p,(char*)p+bytes);if(mprotect(p,page,PROT_READ|PROT_EXEC)){munmap(p,page);return false;}jb.push_back({p,page,(BlockFn)p});ji[pc]=jb.size()-1;return true;}
#endif
 void run(int n){
#ifdef __aarch64__
 ...
 void load(const std::vector<uint8_t>&b){loaded=false;if(b.size()<16||memcmp(b.data(),"NES\x1a",4))return;uint8_t np=b[4],nc=b[5];uint8_t mapper=(b[6]>>4)|(b[7]&0xf0);if(mapper)return;size_t off=16+((b[6]&4)?512:0),ps=np*16384,cs=nc*8192;if(!ps||b.size()<off+ps)return;prg.assign(b.begin()+off,b.begin()+off+ps);off+=ps;chr.assign(cs?cs:8192,0);if(cs&&b.size()>=off+cs)std::copy(b.begin()+off,b.begin()+off+cs,chr.begin());loaded=true;reset();}
...
extern "C" JNIEXPORT jstring JNICALL Java_com_dokcmonika90_retrophone_MainActivity_nativeVersion(JNIEnv*env,jobject){return env->NewStringUTF("NES renderer + audio + ARM64 dynamic recompiler 0.5");}
