...
using BlockFn=void(*)(CPU*);struct JitBlock{void*mem=nullptr;size_t size=0;BlockFn fn=nullptr;};
struct Emulator{
 ...
 std::array<int32_t,65536>jitIndex{};std::vector<JitBlock>blocks;std::array<uint32_t,W*H>frame{};double audioPhase=0,audioFreq=0;uint8_t audioVol=0;std::mutex mu;
 Emulator(){jitIndex.fill(-1);blocks.reserve(1024);}~Emulator(){for(auto&b:blocks)if(b.mem)munmap(b.mem,b.size);}
 uint8_t read(uint16_t a){if(a<0x2000)return cpu.ram[a&0x7ff];if(a<0x4000)return ppuRead(0x2000|(a&7));if(a==0x4016)return readController();if(a>=0x8000&&!prg.empty()){size_t o=a-0x8000;o%=prg.size();return prg[o];}return 0;}
 ...
 uint8_t vramRead(uint16_t a){a&=0x3fff;if(a>=0x3f00)return palette[(a-0x3f00)&31];return vram[a&0xfff];}void vramWrite(uint16_t a,uint8_t v){a&=0x3fff;if(a>=0x3f00)palette[(a-0x3f00)&31]=v&0x3f;else vram[a&0xfff]=v;}
 ...
 void setZN(uint8_t v){if(v==0)cpu.p|=2;else cpu.p&=~2;if(v&0x80)cpu.p|=0x80;else cpu.p&=~0x80;}uint16_t zp(){return read(cpu.pc++);}uint16_t abs(){uint16_t l=read(cpu.pc++),h=read(cpu.pc++);return l|(h<<8);}uint8_t imm(){return read(cpu.pc++);}void push(uint8_t v){cpu.ram[0x100+cpu.sp--]=v;}uint8_t pop(){return cpu.ram[0x100+(++cpu.sp)];}
 ...
 void runCPU(int n){
#ifdef __aarch64__
  for(int i=0;i<n;i++){uint16_t pc=cpu.pc;if(compile(pc)&&jitIndex[pc]>=0)blocks[jitIndex[pc]].fn(&cpu);else step();}
#else
 ...
 void reset(){cpu=CPU{};std::fill(std::begin(cpu.ram),std::end(cpu.ram),0);ppuStatus=0xA0;ppuCtrl=ppuMask=0;addrLatch=false;ppuAddr=0;std::fill(std::begin(vram),std::end(vram),0);std::fill(std::begin(palette),std::end(palette),0);std::fill(frame.begin(),frame.end(),0xff000000);audioPhase=audioFreq=0;audioVol=0;jitIndex.fill(-1);if(!prg.empty())cpu.pc=(uint16_t)read(0xfffc)|((uint16_t)read(0xfffd)<<8);}
 uint32_t color(uint8_t i){static const uint32_t p[64]={0xff666666,0xff002a88,0xff1412a7,0xff3b00a4,0xff5f006b,0xff7e003c,0xff7b0800,0xff5c1d00,0xff344000,0xff0b4f00,0xff005500,0xff00504a,0xff003c63,0,0,0,0xffadadad,0xff155fd9,0xff4240ff,0xff7527fe,0xffa01acc,0xffb71e5b,0xffb52f16,0xff994800,0xff6b6d00,0xff388700,0xff0d9300,0xff008b7a,0xff00749e,0,0,0,0xffffffff,0xff64b0ff,0xff9290ff,0xffc76fff,0xfff36aff,0xfffe6e92,0xffff9b5b,0xfffdbd43,0xffcfe04a,0xff9af57a,0xff5cf0b0,0xff5de1d4,0xff60cfff,0xff808080,0,0,0,0xffffffff,0xffc0dfff,0xffd0d0ff,0xffe8bfff,0xffffb8ff,0xffffb8ff,0xffffcfa0,0xffffe0a0,0xffe8f090,0xffc8ff80,0xffa0ffc0,0xffa0fff0,0xffa0e8ff,0xffc0c0c0,0,0};return p[i&63];}
 void render(){for(int y=0;y<H;y++)for(int x=0;x<W;x++){int tx=((x+scrollX)>>3)&31,ty=((y+scrollY)>>3)&29;uint8_t tile=vram[(ty*32+tx)&0x3ff];int px=(x+scrollX)&7,py=(y+scrollY)&7;size_t off=(size_t)tile*16+py;uint8_t lo=chr.empty()?0:chr[off%chr.size()],hi=chr.empty()?0:chr[(off+8)%chr.size()];uint8_t bit=((lo>>(7-px))&1)|(((hi>>(7-px))&1)<<1);uint8_t pi=bit?((uint8_t)((((tile>>2)&3)*4+bit)&31)):0;frame[y*W+x]=color(palette[pi]);}ppuStatus|=0x80;}
 void demo(){for(int y=0;y<H;y++)for(int x=0;x<W;x++){uint8_t v=((x>>4)^(y>>4))&15;frame[y*W+x]=0xff000000|(v*17<<16)|(v*9<<8)|(v*5);}}
 void runFrame(){if(!loaded){demo();return;}runCPU(9932);render();}void setButtons(uint8_t m){controller=m;}std::vector<int16_t>audio(){std::vector<int16_t>o(735);if(!loaded||audioFreq<=1)return o;double inc=audioFreq/44100.;for(auto&s:o){audioPhase+=inc;if(audioPhase>=1)audioPhase-=1;s=(int16_t)((audioPhase<.5?1:-1)*audioVol*1500);}return o;}
};static Emulator emu;
}
extern "C" JNIEXPORT jstring JNICALL Java_com_dokcmonika90_retrophone_MainActivity_nativeVersion(JNIEnv*e,jobject){return e->NewStringUTF("NES renderer + audio + ARM64 dynamic recompiler 0.4");}
...
extern "C" JNIEXPORT jshortArray JNICALL Java_com_dokcmonika90.retrophone_MainActivity_nativeAudio(JNIEnv*e,jobject){return nullptr;}
extern "C" JNIEXPORT jboolean JNICALL Java_com_dokcmonika90.retrophone_MainActivity_nativeIsLoaded(JNIEnv*,jobject){return JNI_FALSE;}
