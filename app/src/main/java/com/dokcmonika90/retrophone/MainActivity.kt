...
class MainActivity : Activity() {
    ...
    private fun validHeader(b:ByteArray):Boolean=b.size>=16&&b[0].toInt()=='N'.code&&b[1].toInt()=='E'.code&&b[2].toInt()=='S'.code&&b[3].toInt()==0x1A
    ...
    private fun saveToLibrary(bytes:ByteArray,name:String){try{val f=libraryFile(bytes);f.writeBytes(bytes);libraryPrefs.edit().putString("name_${f.name}",name.substringAfterLast('/')).putString("last_rom",f.name).putLong("played_${f.name}",System.currentTimeMillis()).apply()}catch(_:Exception){}}
    private fun markPlayed(file:java.io.File){libraryPrefs.edit().putString("last_rom",file.name).putLong("played_${file.name}",System.currentTimeMillis()).apply();file.setLastModified(System.currentTimeMillis())}
    private fun autoLoadLastRom(){val name=libraryPrefs.getString("last_rom",null)?:return;val file=java.io.File(romDirectory,name);if(!file.isFile)return;try{val bytes=file.readBytes();if(validHeader(bytes)){loadRomBytes(bytes,libraryPrefs.getString("name_${file.name}",file.name)?:file.name,false)}}catch(_:Exception){}}
    private fun showLibrary(){val files=romDirectory.listFiles{f->f.isFile&&f.extension.equals("nes",true)}?.sortedByDescending{libraryPrefs.getLong("played_${it.name}",it.lastModified())}?:emptyList();val box=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(18),dp(8),dp(18),dp(8))};if(files.isEmpty())box.addView(TextView(this).apply{text="No games saved yet.\nUse LOAD NES to add a game.";textSize=17f;setPadding(0,dp(16),0,dp(16))})else files.forEach{file->val row=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(0,dp(6),0,dp(6))};val name=libraryPrefs.getString("name_${file.name}",file.name)?:file.name;val bytes=try{file.readBytes()}catch(_:Exception){ByteArray(0)};val mapper=if(bytes.size>=8)((bytes[6].toInt()and 0xF0)shr 4)or(bytes[7].toInt()and 0xF0)else -1;val info=TextView(this).apply{text="$name\nMapper $mapper • ${bytes.size/1024} KB";textSize=16f};val buttons=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL};val play=Button(this).apply{text="PLAY"};play.setOnClickListener{loadRomBytes(file.readBytes(),name,false);markPlayed(file);showLibrary()};val del=Button(this).apply{text="DELETE"};del.setOnClickListener{file.delete();libraryPrefs.edit().remove("name_${file.name}").remove("played_${file.name}").apply();if(libraryPrefs.getString("last_rom",null)==file.name)libraryPrefs.edit().remove("last_rom").apply();showLibrary()};buttons.addView(play);buttons.addView(del);row.addView(info);row.addView(buttons);box.addView(row)};AlertDialog.Builder(this).setTitle("ROM Library").setView(box).setNegativeButton("CLOSE",null).show()}
    private fun loadRomBytes(bytes:ByteArray,name:String,save:Boolean){val mapper=if(bytes.size>=8)((bytes[6].toInt()and 0xF0)shr 4)or(bytes[7].toInt()and 0xF0)else -1;try{if(save)saveToLibrary(bytes,name);val ok=nativeLoad(bytes);if(ok){nativeReset();status.text="  ROM loaded: $name (Mapper $mapper, ${bytes.size/1024} KB)";screen.invalidate()}else{status.text="  Mapper $mapper is not implemented by the current core";Toast.makeText(this,"LaiNES could not load Mapper $mapper",Toast.LENGTH_LONG).show()}}catch(e:Exception){status.text="  ROM load failed";Toast.makeText(this,"ROM load failed: ${e.message?:"unknown error"}",Toast.LENGTH_LONG).show()}}
