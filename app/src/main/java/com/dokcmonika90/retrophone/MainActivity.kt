...
class MainActivity : Activity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        root.addView(toolbar, FrameLayout.LayoutParams(-1,dp(62),Gravity.TOP)); setContentView(root); startAudio(); startLoop()
    }
    ...
    private fun saveToLibrary(bytes:ByteArray,name:String){try{val f=libraryFile(bytes);f.writeBytes(bytes);libraryPrefs.edit().putString("name_${f.name}",name.substringAfterLast('/')).putString("last_rom",f.name).apply()}catch(_:Exception){}}
    private fun showLibrary(){val files=romDirectory.listFiles{f->f.isFile&&f.extension.equals("nes",true)}?.sortedByDescending{it.lastModified()}?:emptyList();val box=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(18),dp(8),dp(18),dp(8))};if(files.isEmpty())box.addView(TextView(this).apply{text="No games saved yet.\nUse LOAD NES to add a game.";textSize=17f;setPadding(0,dp(16),0,dp(16))})else files.forEach{file->val row=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(0,dp(5),0,dp(5))};val name=libraryPrefs.getString("name_${file.name}",file.name)?:file.name;val label=TextView(this).apply{text=name;textSize=16f;setPadding(0,dp(8),dp(8),dp(8))};val play=Button(this).apply{text="PLAY"};play.setOnClickListener{loadRomBytes(file.readBytes(),name,false);file.setLastModified(System.currentTimeMillis());showLibrary()};val del=Button(this).apply{text="DELETE"};del.setOnClickListener{file.delete();libraryPrefs.edit().remove("name_${file.name}").apply();showLibrary()};row.addView(label,LinearLayout.LayoutParams(0,-2,1f));row.addView(play);row.addView(del);box.addView(row)};AlertDialog.Builder(this).setTitle("ROM Library").setView(box).setNegativeButton("CLOSE",null).show()}
